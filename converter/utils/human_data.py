...
_GPU_DEVICE = torch.device('cuda') if torch.cuda.is_available() else None

SMPL_PRESET = {'type': dict, 'model_type': 'smpl',
               'kwargs': dict(num_betas=10)}
SMPLX_PRESET = {'type': dict, 'model_type': 'smplx', 
                'kwargs': dict(num_betas=10, flat_hand_mean=False, 
...
_HumanData_SUPPORTED_KEYS = {
    # Here defines the basic structure of HumanData.
    # Use metadata to store dataset info and config to define annotation info included.
    'metadata': {
        'type': dict,
        'keys': ['dataset', 'subset', 'date', 'version', 'slice_number', 'slice_index'],
        'keys_type': [str, str, str, str, int, int],
        'default_value': [None, None, get_today_date(), None, 1, 1],
    },
    'config': {
        'type': dict,
        'keys': ['path', 'model', 'keypoint', 'bbox', 'camera', 'temporal', 'dataset_specific'],
        'keys_type': [list, dict, list, list, list, list, list],
        'default_value': [['image'], SMPLX_PRESET, ['2d'], ['body'], ['intrinsics'], [], []], 
    },
    'path': {
        'type': dict,
        'allowed_keys': ['image', 'depth', 'pointcloud'],
        'allowed_keys_type': [list], 
    },
    'smpl': {
        'type': dict,
        'keys': ['betas', 'body_pose', 'global_orient', 'transl'],
        'allowed_keys_type': [np.ndarray],
        'shape': smpl_shape,
    },
    'smplx': {
        'type': dict,
        'keys': ['betas', 'transl', 'global_orient', 'body_pose'], 
        'allowed_keys': ['left_hand_pose', 'right_hand_pose', 'leye_pose', 'reye_pose', 'jaw_pose', 'expression'],
        'allowed_keys_type': [np.ndarray],
        'shape': smplx_shape,
    },
    'keypoint': {
        'type': dict,
        'allowed_keys': ['2d', '3d', '2d_convention', '3d_convention',
                         '2d_smpl', '2d_smplx', '3d_smpl', '3d_smplx'],
        'allowed_keys_type': [np.ndarray, str],
    },
    'bbox': {
        'type': dict,
        'keys': ['type'],
        'keys_type': [str],
        'default_value': ['xywh'],
        'allowed_keys': ['body', 'left_hand', 'right_hand', 'face'],
        'allowed_keys_type': [np.ndarray],
    },
    'camera': {
        'type': dict,
        'allowed_keys': ['intrinsics', 'extrinsics'],
        'allowed_keys_type': [np.ndarray],
    },
    'temporal': {
        'type': dict,
        'allowed_keys': ['frame_id', 'track_id', 'camera_view', 'sequence_name'],
        'allowed_keys_type': [int, str],
    },
}
